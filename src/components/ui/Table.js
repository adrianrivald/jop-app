export default function Table({
  ...
  const TableItem = (props) => (
    <table
      className={`${isWithHeader && !props.isTableFooter ? 'border-t-2' : ''} ${
        isWithFooter && !props.isTableFooter ? 'border-b-2 ' : ''
      } border-bgrey border-spacing-1 bg-bgrey w-full text-sm text-left text-gray-500 dark:text-gray-400 ${
      ...
      <thead className={`text-xs ${backgroundColor}`}>
        <tr>
          <th scope="col" className={`py-3 px-2 ${cellBorder}`}>
            <div className="my-2 font-normal text-xxs">Divisi</div>
            <div>{props.divisi_item}</div>
          </th>
          <th scope="col" className={`py-3 px-2 ${props.isTableFooter ? '' : 'border-l-2'} ${cellBorder}`}>
            <div className="my-2 font-normal text-xxs">Hancak</div>
            <div>{props.hancak_item}</div>
          </th>
          <th scope="col" className={`py-3 px-2 ${props.isTableFooter ? '' : 'border-l-2'} ${cellBorder}`}>
            <div className="my-2 font-normal text-xxs">Block</div>
            <div>{props.block_item}</div>
          </th>
          <th scope="col" className={`py-3 px-2 ${props.isTableFooter ? '' : 'border-l-2'} ${cellBorder}`}>
            <div className="my-2 font-normal text-xxs">Clone</div>
            <div>{props.clone_item}</div>
          </th>
          <th scope="col" className={`py-3 px-2 ${props.isTableFooter ? '' : 'border-l-2'} ${cellBorder}`}>
            <div className="my-2 font-normal text-xxs">Sistem</div>
            <div>{props.sistem_item}</div>
          </th>
        </tr>
      </thead>
    </table>
  ...
  const TableFooter = (props) => (
    <table
      className={`${isWithHeader && !props.isTableFooter ? 'border-t-2' : ''} ${
        isWithFooter && !props.isTableFooter ? 'border-b-2 ' : ''
      } border-bgrey border-spacing-1 bg-bgrey w-full text-sm text-left text-gray-500 dark:text-gray-400 cursor-pointer`}
    >
      <thead className="text-xs bg-white">
        <tr>
          <th scope="col" className={`py-3 px-2 w-5/12 border-bgrey`}>
            <div className="my-2 font-normal text-xxs">Mandor</div>
            <div className="text-sm">{props.mandor_item}</div>
          </th>
          <th scope="col" className={`py-3 px-2 border-bgrey`}>
            <div className="my-2 font-normal text-xxs">Tapper</div>
            <div>
          ...
          <th scope="col" className={`py-3 px-2 border-bgrey`}>
            ...
            <div className="text-xs">{props.tanggal_tugas_item} - selesai</div>
          </th>
        </tr>
      </thead>
    </table>
  ...
  const Header = (props) => (
    <div className="flex justify-between items-center py-3 px-2 bg-white">
      <span className="text-xs">
        {props.title} :<span className="font-bold text-lg">&nbsp;{props.titleItem}</span>
      </span>
      <span className="font-bold">{props.titleCode}</span>
    </div>
  );
