export default function Toast ({isShow = false, text, onClose}) {
    return (
        <div id="toast-success" className={`absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 transition-all duration-200 flex items-center p-4 mb-4 w-full max-w-xs text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 ${isShow === true ? 'visible opacity-100  top-10' : 'invisible opacity-0 top-0'}`} role="alert">
            ...
            <button onClick={onClose} type="button" class="ml-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700" data-dismiss-target="#toast-success" aria-label="Close">
