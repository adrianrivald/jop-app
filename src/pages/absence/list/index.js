  ...
  return props.workerList.map((result, idx) => {
    ...
    return (
      <div className="flex justify-between items-center mt-3 pt-3" key={idx}>
        ...
        <div className="w-40">
          <p className={`font-bold text-sm truncate ${modified_date !== 'Invalid date' ? '' : 'text-earth'}`}>
            {result?.nama}
          </p>
        </div>
        <p className={`w-20 text-sm text-flora font-bold ${modified_date !== 'Invalid date' ? '' : 'text-earth'}`}>
          {modified_date !== 'Invalid date' ? modified_date : '-'}
        </p>
      ...
      <div className="container">
        ...
        <div>
          ...
          <div className="divide-y divide-cloud">
            {workerList.length > 0 ? (
              <WorkerList workerList={workerList} onClickWorker={onClickWorker} />
            ) : (
