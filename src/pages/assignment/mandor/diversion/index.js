...
const MandorDiversionAssignment = () => {
    ...
    const handleDiversionAssignment = async () => {
        await axios.post(`${url}penugasan/pengajuan-pengalihan/${id}
        `, {
            alasan: reason,
            ...
            <section className="container p-4 flex flex-col justify-between" style={{ height: 'calc(100vh - 70px)'}}>
                <div className="flex flex-col" key={detail.id}>
                    ...
                    <div className="flex flex-col items-start w-full mb-4">
                        <label htmlFor="reason" className="text-xs text-black mb-2">Alasan Pengalihan:</label>
                        ...
                        : <select id="reason" className="flex justify-between w-full font-bold bg-white dark:bg-white shadow border-none text-xs rounded-lg focus:ring-flora-500 focus:border-flora-500 block w-full p-3" defaultValue={reason} onChange={(e) => { setReason(e.target.value) }}>
                            {listReason.map((reason, idx) => {
                                return(
                                    <option key={idx} value={reason.value} className="block w-full">{reason.label}</option>
                                )
                            })}
                        </select>}
                    </div>
                    <div className="flex flex-col items-start w-full mb-4">
                        <label htmlFor="additionalMessage" className="text-xs text-black mb-2">Pesan Tambahan (optional):</label>
                        <textarea id="additionalMessage" rows="4" className="flex justify-between w-full font-bold bg-white dark:bg-white shadow border-none text-xs rounded-lg focus:ring-flora-500 focus:border-flora-500 block w-full p-3 resize-none" placeholder="Masukkan pesan disini..." value={detail.pesan_tugas ? detail.pesan_tugas : additional} onChange={(e) => { setAdditional(e.target.value)}} disabled={detail.status_tugas === 'dialihkan' ? true : false}>
                        </textarea>
                    </div>
                </div>
                { detail.status_tugas !== "dialihkan" ? 
                    <FlatButton className={'w-full mb-2 text-sm font-bold'} text={'Ajukan'} onClick={handleDiversionAssignment}/> : null }
            </section>
       </div>
    )
}
