...
const AddBalanced = () => {
  ...
  const onChangeHandler = (e, input_id) => {
    if (input_id === 'bahan_baku_id') {
      setAddInput({
        ...addInput,
        bahan_baku_id: [...addInput['bahan_baku_id'], e.target.value],
      });
    } else {
      setAddInput((prev) => ({
        ...prev,
    ...
    <div className="App min-h-screen h-full">
      ...
      >
        ...
        <Dropdown
          ...
          option={tphList}
          onChange={(e) => onChangeHandler(e, 'tph_id')}
        />
          ...
          {materialList?.map((res, idx) => (
            <li className="flex justify-between items-center my-2">
              ...
              <input
                ...
                onChange={(e) => onChangeHandler(e, 'bahan_baku_id')}
        ...
        <div className="my-4 flex justify-between w-full">
          ...
          <div className="flex-1 flex flex-col mr-4">
            <Dropdown
              title="Hari Hujan"
              defaultValue="Pilih hari hujan"
              option={raindayList}
              onChange={(e) => onChangeHandler(e, 'hari_hujan')}
            />
          </div>
          <div className="flex-1 flex flex-col">
            <Dropdown
              title="Waktu Hujan"
              defaultValue="Pilih waktu hujan"
              option={raintimeList}
              onChange={(e) => onChangeHandler(e, 'waktu_hujan')}
            />
          </div>
        </div>
        <div className="w-full border-b border-flora my-6" />
        <Dropdown
          title="Petugas Timbang"
          defaultValue="Pilih petugas timbang"
          option={weigherList}
        ...
        <div className="flex justify-between items-center my-7 w-full">
          <FlatButton
            role="white"
            className={'w-full text-sm font-bold mr-4 mb-7'}
            text={'Kembali'}
            onClick={() => navigate(-1)}
          />
