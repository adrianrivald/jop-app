...
const Balanced = () => {
    ...
    const onDoneWeighing = (id) => {
        axios.get(`${url}penimbangan/selesai/${id}`, {
            url: process.env.REACT_APP_API_URL,
            headers: {
                Authorization: `Bearer ${token}`,
                Accept: 'application/json'
                            ...
                            <>
                                <div className="flex justify-between mb-6">
                                    <div className="flex flex-col items-start">
                                        <p className="text-xs">{moment(res?.tanggal_penimbangan, 'YYYY-MM-DD hh:mm:ss').format('DD MMMM YYYY, hh:mm')}</p>
                                        <p className="text-sm font-bold">{res?.kode}</p>
                                    </div>
                                    <button className="text-xs font-bold px-4 py-1 text-flora drop-shadow-md bg-white rounded-xl" onClick={onCopyCode}>Salin Kode</button>
                                </div>
                                        ...
                                        res?.detail?.map((res, idx) => {
                                            return (
                                                <div className="flex justify-between items-center">
                                                    <div className="flex-1 text-left"> {res?.kode} - <b>{res?.nama}</b> </div>
                                                    <div className="flex-1 text-right"> <b>{res?.berat_total}</b> kg </div>             
                                                </div>
                                ...
                                <FlatButton className={'w-full mb-4 text-sm flex justify-center mt-6'} text="Selesai" onClick={() => onDoneWeighing(res?.id)}disabled={isButtonDisabled} />
                            </> : null
                        )
                    })}
                                    ...
                                    openedId[`item_${idx}`] === true ? (
                                        <div className="bg-white divide-y divide-cloud">
                                            <div className="flex justify-between items-center p-3 transition-transform cursor-pointer" onClick={() => onCollapse(idx)}>
                                                <div className="flex flex-col justify-start">
                                                    <div className="text-xs">{moment(res?.tanggal_penimbangan, 'YYYY-MM-DD hh:mm:ss').format('DD MMMM YYYY, hh:mm')}</div>
                                            ...
                                            <div className=" p-3 ">
                                                {
                                                    res?.detail?.length > 0 ? res?.detail?.map((res, idx) => {
                                                        return (
                                                            <>
