...
function Logistic () {
    ...
    const getTPH = () => {
        axios.get(`${url}tph/list?include=wilayah_tugas,divisi&sort=kode,nama`, {
    ...
    const getBatchReadyToDeliver = () => {
        axios.get(`${url}pengiriman/batch-siap-kirim?filter[tph]=${selectedTph}&filter[periode_penimbangan]=${dateFrom.concat(',',dateTo)}`, {
            url: process.env.REACT_APP_API_URL,
            headers: {
                Authorization: `Bearer ${token}`,
                Accept: 'application/json'
    ...
    const getBatchOnDelivery = () => {
        axios.get(`${url}pengiriman/batch-dalam-pengiriman?filter[tph]=${selectedTph}&filter[periode_penimbangan]=${dateFrom.concat(',',dateTo)}`, {
            url: process.env.REACT_APP_API_URL,
            headers: {
                Authorization: `Bearer ${token}`,
                Accept: 'application/json'
    ...
    const getBatchDelivered = () => {
        axios.get(`${url}pengiriman/batch-selesai-kirim?filter[tph]=${selectedTph}&filter[periode_penimbangan]=${dateFrom.concat(',',dateTo)}`, {
            url: process.env.REACT_APP_API_URL,
            headers: {
                Authorization: `Bearer ${token}`,
                Accept: 'application/json'
    ...
    return (
        <>
            ...
            <div className="container">
                <Dropdown title="Masukan kode lokasi awal" defaultValue="Pilih kode lokasi awal" option={tphList} onChange={(e) => onChangeTph(e)} />
                <div>
                ...
                <div className="batch mt-5">
                    ...
                    {
                        batchOnDelivery?.length > 0 ? batchOnDelivery?.map((res, idx) => {
                            return (
                                <div className="flex justify-between items-center mt-3">
                                    <div className="w-2/4 cursor-pointer" onClick={() => onClickBatch(res?.id)}>
                                        <div className="flex items-center">
                ...
                <div className="batch mt-5">
                    ...
                    {
                        batchDelivered?.length > 0 ? batchDelivered?.map((res, idx) => {
                            return (
                                <div className="flex justify-between items-center mt-3">
                                    <div className="w-2/4 cursor-pointer" onClick={() => onClickBatch(res?.id)}>
                                        <div className="flex items-center">
