...
function LogisticLoading () {
    ...
    const handleSubmit = async() => {
        ...
        await axios.post(`${url}pengiriman/loading/store-new
        `, payload, config).then(() => {
            setIsSubmitted(true)
        ...
        <>
            ...
            <div className="container h-screen relative">
                <div className="load-detail">
                    <p className="font-bold">{batchItem?.batchCode} ({batchItem?.name})</p>
                    <div className="flex justify-between mt-3 gap-3">
                        <div className="flex-auto w-1/3 p-3 rounded-xl border border-cloud w-full">
                            <p className="text-xxs mb-1">Berat total</p>
                            <span className="text-4xl font-bold">{batchItem?.weight}</span><span> kg</span>
                        </div>
                        <div className="flex-auto w-1/3 p-3 rounded-xl border border-cloud w-full">
                            ...
                            <span className="text-4xl font-bold">{batchItem?.remain_weight}</span><span> kg</span>
                        </div>
                        <div className="flex-auto w-1/3 p-3 rounded-xl border border-cloud w-full">
                            <p className="text-xxs">Berat dikirim</p>
                            <div className='relative'>
                                <span className='absolute inset-y-6 right-4'>kg</span>
                                <input className="w-full text-4xl font-bold mt-1 rounded-lg px-3 text-xs focus:outline-none focus:shadow-outline" type="text" pattern="\d*" maxLength="3" onChange={(e) => onChangeHandler(e, 'berat_kirim')}/>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mt-3">
                        <label htmlFor="file-upload" className="w-full rounded-lg bg-flora text-white font-bold p-3 block text-center flex justify-center items-center gap-2 cursor-pointer">
