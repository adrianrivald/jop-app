...
function CheckOut(props) {
  ...
  const [openedDoneId, setOpenedDoneId] = React.useState();
  ...
  const [stockSalesDoneData, setStockSalesDoneData] = React.useState([]);
  ...
  const getStockSalesDone = () => {
    axios
      .get(
        `${url}/warehouse/stock-selesai-checkout?filter[gudang]=${selectedGudang}&filter[status_kirim]=null|dalam-pengiriman|selesai-terkirim`,
        {
          url: process.env.REACT_APP_API_URL,
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  React.useEffect(() => {
    if (selectedGudang !== '') {
      ...
      getStockSalesDone(selectedGudang);
    }
  }, [selectedGudang]);
    ...
    if (!selectedClient) {
      ...
      await axios
        ...
        .then(() => {
          ...
          setTimeout(() => {
            localStorage.removeItem('check-out-value');
            setIsButtonDisabled(false);
            setIsSubmitted(false);
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mt-8">
        <p className="text-sm font-bold">Daftar Order Sales Selesai</p>
      </div>
      <div className="mt-7">
        {stockSalesDoneData?.length > 0 ? (
          stockSalesDoneData?.map((res, idx) =>
            openedDoneId === idx + 1 ? (
              <div>
                <div className="bg-white">
                  <div
                    className="flex justify-between items-center transition-transform cursor-pointer pt-3 pl-3 pr-3"
                    onClick={() => setOpenedDoneId(0)}
                  >
                    <div className="flex items-center">
                      <div>
                        <p className="font-bold">{res?.kode}</p>
