...
function MandorWeighing() {
  ...
  const onCheckMaterial = (code, name, id, id_bahan_baku) => {
    if (!selectedMaterial.find((val) => val.id === id)) {
      ...
      setSelectedMaterial(selectedMaterial.filter((val) => val.id !== id));
    ...
    <>
      ...
      <div className="container">
        <div>
          ...
          />
          <Dropdown
            title="Kode Batch Penimbangan"
            defaultValue="Pilih Kode Batch Penimbangan"
            option={weighingList}
            onChange={(e) => onChangeWeighing(e)}
          />
              ...
              <div className="mt-5">
                ...
                {weighingDetail?.map((res, idx) => (
                  <div className="flex justify-between gap-2 mt-4">
                    ...
                    <input
                      checked={selectedMaterial.find((val) => val.id === idx)}
                      onClick={() => onCheckMaterial(res.kode, res.nama, idx, res?.id)}
                      type="checkbox"
                      className="accent-flora w-4 h-4 text-flora bg-gray-100 rounded border-gray-300 dark:bg-gray-700 dark:border-gray-600"
                    />
                  </div>
